...
namespace events.tac.local.Business
{
    public class NavigationBuilder
    {
        ...
        public NavigationMenuItem Build()
        {
            ...
            return new NavigationMenuItem(root?.DisplayName, root?.Url, root != null && _context?.ContextItem != null ? Build(root, _context.ContextItem) : null);
        }

        private IEnumerable<NavigationMenuItem> Build(IItem node, IItem current)
        {
            return node.GetChildren().Select(i => new NavigationMenuItem(i.DisplayName, i.Url, i.IsAncestorOrSelf(current) ? Build(i, current) : null));
        }
    }
}